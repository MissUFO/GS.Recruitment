CREATE PROCEDURE [process].[Tasks_List_UserFrom]
...
BEGIN
	...
	SET @Xml = (SELECT (SELECT task.Id
						  ...
						  FROM [process].[Tasks] AS task
						  		LEFT OUTER JOIN auth.Users userfrom ON userfrom.Id = task.UserFromId 
								LEFT OUTER JOIN auth.Users userto ON userto.Id = task.UserToId
						  WHERE task.UserFromId = @UserFromId
